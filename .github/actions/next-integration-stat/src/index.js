...
async function fetchJobLogsFromWorkflow(octokit, token, job) {
  ...
  const nextjsVersion = dateTimeStripped.find((x) => x.includes("RUNNING NEXTJS VERSION:") && !x.includes("$("))?.split("RUNNING NEXTJS VERSION:").pop()?.trim();
...
async function run() {
  ...
  for (const job of integrationTestJobs) {
    const {logs, nextjsVersion} = await fetchJobLogsFromWorkflow(octokit, token, job);
    ...
    fs.writeFileSync('./nextjs-test-results.json', JSON.stringify(testResultsManifest, null, 2));
  }
